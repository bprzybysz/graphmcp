...
def log_step_start(
    workflow_id: str, step_name: str, step_description: str = None
) -> str:
    """
    ...
    return log.append_log(
        spinner_message,
        level="info",
        metadata={"step": step_name, "status": "in_progress"},
    )
...
def log_step_complete(
    ...
    log = get_workflow_log(workflow_id)
    return log.update_log_entry(
        entry_id,
        complete_message,
        {"step": step_name, "status": "completed" if success else "failed"},
    )
