...
class TestUIComponents(unittest.TestCase):
    ...
    def test_workflow_controls_renders_start_button(
        self, mock_st, mock_session_manager
    ):
        """
        ...
        stop_call = any(
            "Stop Workflow" in call[0][0] for call in mock_st.button.call_args_list
        )
    ...
    def test_workflow_controls_renders_stop_button(self, mock_st, mock_session_manager):
        ...
        start_call = any(
            "Start Workflow" in call[0][0] for call in mock_st.button.call_args_list
        )
        self.assertFalse(start_call, "Start button should not be rendered")

    @patch("ui.components.workflow_controls.SessionManager")
    @patch("ui.components.workflow_controls.st")
    def test_workflow_controls_renders_toggle_and_progress(
        self, mock_st, mock_session_manager
    ):
        """
        ...
        # Assert
        mock_st.toggle.assert_called_with(
            "Auto-refresh",
            value=True,
    ...
    @patch("ui.components.log_stream.SessionManager")
    @patch("ui.components.log_stream.st")
    ...
    @patch("ui.components.progress_tables.SessionManager")
    @patch("ui.components.progress_tables.st")
    @patch("ui.components.progress_tables.pd")
