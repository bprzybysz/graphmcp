...
async def _perform_cleanup(
    github_client: GitHubMCPClient, fork_owner: str, fork_name: str, branch_name: str
):
    try:
        ...
        print(
            f"Cleanup: Failed to delete branch {branch_name} on {fork_owner}/{fork_name}: {e}"
        )
    ...
    try:
        ...
        print(
            f"Cleanup: Failed to delete forked repository {fork_owner}/{fork_name}: {e}"
        )
